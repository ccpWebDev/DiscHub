...
$name = $uname = $pass = $degree = $phone = "";
$name_err = $uname_err = $pass_err = $degree_err = $phone_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["uname"]))) {
        ...
        $sql = "SELECT id FROM tbl_teacher WHERE username = ?";
    ...
    if (empty($_POST["degree"])) {
        $degree_err = "Please enter the degree.";
    } else {
        $degree = $_POST["degree"];
    }
    ...
    if (empty($name_err) && empty($uname_err) && empty($pass_err) && empty($degree_err) && empty($phone_err)) {
        // Prepare an insert statement
        $sql = "INSERT INTO tbl_teacher (name, username, password, degree, phone) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_username, $param_password, $param_degree, $param_phone);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo "Oops! Something went wrong. Please try again later.";
                echo "Error: " . mysqli_error($conn); // Print the SQL error message
            }
    ...
    <div class="bg-white overflow-hidden shadow-md rounded-lg border border-solid border-blue-500 rounded-lg p-4"
        ...
        <form class="max-w-md mx-auto" action="" method="POST">
            ...
            <div class="mb-4">
                ...
                <span class="text-red-500"><?php echo $uname_err; ?></span>
            </div>
            <div class="mb-4">
                ...
                <span class="text-red-500"><?php echo $pass_err; ?></span>
            </div>
            <div class="mb-4">
                ...
                <span class="text-red-500"><?php echo $degree_err; ?></span>
            </div>
            <div class="mb-4">
